...
class SoldStrategyType(IntFlag):
    SoldLossStopLowerThanAvgPrice = 1         # 股价低于平均成交价格线
    SoldLossStopAvgPriceGoLow = 2             # 平均成交价格线走低
    SoldLossStopHighThanAvgPrice = 4          # 股价高于平均成交价格线一定幅度
    SoldProfitStopLowerThanAvgPrice = 8       # 股价低于于平均成交价格线一定幅度止盈
    SoldProfitStopLowerThanDeadLine = 16      # 涨停/跌停，第二天卖出
    SoldLossStopNeedBuyStock = 32             # 可以先卖后买，腾出仓位
    SoldNextDay = 32     
...
class SoldInfoObserver():
     def __init__(self):
        self.first_limit_time = None
        self.is_sell = False
        self.sold_strategy_type = None
        self.strong_level = 0
...
class StrategyTaskYugiS1(StrategyTask):
    ...
    def __init__(self, trade_date: str, mode: StrategyTaskMode = StrategyTaskMode.MODE_BACKTEST_LOCAL,
        ...
        self.observer_map_to_buy: Dict[str, BuyInfoObserver] = {}

        self.observe_stocks_to_sell: Dict[str, TradeRecordS1] = {}
        self.observer_map_to_sell: Dict[str, SoldInfoObserver] = {}
    ...
    def prepare_observed_pool(self):
        ...
        with Session() as session:
            ...
            self.observer_map_to_buy = {stock_code: BuyInfoObserver() for stock_code in self.observe_stocks_to_buy.keys()}
            self.observer_map_to_sell = {stock_code: SoldInfoObserver() for stock_code in self.observe_stocks_to_sell.keys()}
            pre_stock_daily_results = session.query(StockDailyData).filter(
                StockDailyData.ts_code.in_(self.observer_map_to_sell.keys()),
            ...
            for pre_daily_data in pre_stock_daily_results:
                observer_sold_info: Optional[SoldInfoObserver] = self.observer_map_to_sell[pre_daily_data.ts_code]
    ...
    def handle_buy_stock(self, stock_code: str, minute_data, trade_time: str):
        ...
        observer_map_to_buy = self.observer_map_to_buy
        if stock_code not in observer_map_to_buy:
            return
        observer_buy_info = observer_map_to_buy[stock_code]
                ...
                if close_price >= observer_buy_info.buy_threshold and close_price >= avg_price:
                    ...
                    for other_stock_code, other_stock in self.observe_stocks_to_buy.items():
                        if other_stock.concept_name == observed_stock.concept_name and other_stock_code != observed_stock.stock_code:
                            observer_map_to_buy[other_stock_code].stop_buy = True
        ...
        if observed_stock.concept_position == '龙一' and curent_open_rate < buy_in_observe_dead_pch:
            ...
            for other_stock in self.observe_stocks_to_buy.values():
                if other_stock.concept_name == observed_stock.concept_name:
                    other_info = observer_map_to_buy[other_stock.stock_code]
    ...
    def handle_sell_stock(self, stock_code: str, minute_data, trade_time: str):
        ...
        observer_map_to_sell = self.observer_map_to_sell
        if stock_code not in observer_map_to_sell:
            return
        observer_sold_info: Optional[SoldInfoObserver] = observer_map_to_sell[stock_code]
        if observer_sold_info.is_sell:
            return
        ...
        if observer_sold_info.sold_strategy_type == None:
            # 指定卖出策略
            pre_stock_daily_data: Optional[StockDailyData] = pre_stock_daily_map[stock_code]
            open_rate = (open / observer_sold_info.pre_close - 1)*100 

            if open_rate <= limit_down_pch:
                observer_sold_info.strong_level = -2
                observer_sold_info.sold_strategy_type = SoldStrategyType.SoldLossStopHighThanAvgPrice
            elif open_rate <= -4:
                observer_sold_info.strong_level = -1
                observer_sold_info.sold_strategy_type = SoldStrategyType.SoldLossStopLowerThanAvgPrice | SoldStrategyType.SoldLossStopAvgPriceGoLow | SoldStrategyType.SoldLossStopNeedBuyStock
                ...
                if pre_stock_daily_data.close < pre_stock_daily_data.high:
                    ...
                    # 涨停板
                    observer_sold_info.strong_level = 1
                    observer_sold_info.sold_strategy_type = SoldStrategyType.SoldProfitStopLowerThanDeadLine |  SoldStrategyType.SoldProfitStopLowerThanAvgPrice
                    if stock_code in self.pre_limit_data_source.limit_up_map and open_rate < 0:
                        ...
                        if self.is_strong_limit_stock(pre_limit_data):
                            # WechatBot.send_text_msg(f"{stock_code} {self.trade_date}")
                            observer_sold_info.sold_strategy_type = observer_sold_info.sold_strategy_type | SoldStrategyType.SoldLossStopNeedBuyStock | SoldStrategyType.SoldLossStopLowerThanAvgPrice | SoldStrategyType.SoldLossStopAvgPriceGoLow
            elif (pre_stock_daily_data.high / pre_stock_daily_data.pre_close - 1)*100<=limit_up_pch:
                # 未封板
                observer_sold_info.strong_level = -1
                observer_sold_info.sold_strategy_type = SoldStrategyType.SoldLossStopLowerThanAvgPrice | SoldStrategyType.SoldLossStopAvgPriceGoLow | SoldStrategyType.SoldLossStopNeedBuyStock
            
            logging.info(f"{trade_time} {stock_code} 卖出策略 {observer_sold_info.sold_strategy_type}")
            
        logging.debug(f"卖出回调 {trade_time} {stock_code} {open} {close} {avg} {round((high - avg)*100/observer_sold_info.pre_close, 2)}")
        sold_strategy_type = observer_sold_info.sold_strategy_type
        if trade_time > '09:31':
            if (sold_strategy_type & SoldStrategyType.SoldLossStopLowerThanAvgPrice) and open - avg < 0 and close - avg < 0:
                sell_reason = f'基于策略{SoldStrategyType.SoldLossStopLowerThanAvgPrice} 且低于分时黄线'
                logging.info(f"卖出 {stock_code} {trade_time} {close} {avg} {observer_sold_info.pre_avg_price} {sell_reason}")
                self.sell_stock(stock_code, close, current_time, observer_sold_info, sell_reason)
                return
            if (sold_strategy_type & SoldStrategyType.SoldLossStopAvgPriceGoLow) and observer_sold_info.pre_avg_price != None and avg - observer_sold_info.pre_avg_price < -0.01:
                sell_reason = f'基于策略{SoldStrategyType.SoldLossStopAvgPriceGoLow} 且分时黄线下行'
                logging.info(f"卖出 {stock_code} {trade_time} {open} {avg} {observer_sold_info.pre_avg_price} {sell_reason}")
                self.sell_stock(stock_code, open, current_time, observer_sold_info, sell_reason)
                return
            if (sold_strategy_type & SoldStrategyType.SoldProfitStopLowerThanAvgPrice):
                if open < avg:
                    if observer_sold_info.under_water_minutes == 0:
                        observer_sold_info.under_water_avg_price_start = avg
                    observer_sold_info.under_water_minutes += 1
                elif open > avg:
                    observer_sold_info.under_water_minutes = 0
                if observer_sold_info.under_water_minutes > under_water_stop_minutes or (observer_sold_info.under_water_avg_price_start and ( avg/observer_sold_info.under_water_avg_price_start-1)*100 < under_water_avg_price_pch_limit):
                    sell_reason = f'基于水下时长{observer_sold_info.under_water_minutes}分钟'
                    logging.info(f"卖出 {stock_code} {trade_time} {open} {current_time} {sell_reason} {avg} {observer_sold_info.under_water_avg_price_start} {observer_sold_info.pre_close}")
                    self.sell_stock(stock_code, open, current_time, observer_sold_info, sell_reason)
                    return
            if (sold_strategy_type & SoldStrategyType.SoldProfitStopLowerThanDeadLine) and (open / observer_sold_info.pre_close - 1) * 100 < profit_stop:
                sell_reason = f'基于{profit_stop}%止盈'
                logging.info(f"卖出 {stock_code} {trade_time} {open} {current_time} {sell_reason}，水下时长{observer_sold_info.under_water_minutes}分钟 {avg} {observer_sold_info.under_water_avg_price_start}")
                self.sell_stock(stock_code, open, current_time, observer_sold_info, sell_reason)
                return
        if trade_time != "09:30":
            if (sold_strategy_type & SoldStrategyType.SoldLossStopHighThanAvgPrice) and (high - avg)*100/observer_sold_info.pre_close >= sold_out_pch:
                sell_reason = f'基于分时偏离平均价格 {sold_out_pch}%'
                logging.info(f"卖出 {stock_code} {trade_time} {close} {trade_time} {sell_reason}")
                self.sell_stock(stock_code, close, current_time, observer_sold_info, sell_reason)
                return
        if trade_time >= "14:54":
            current_pch = (open / observer_sold_info.pre_close - 1) * 100
        ...
        first_limit_time = observer_sold_info.first_limit_time
        if not first_limit_time and high == low and (high / observer_sold_info.pre_close - 1) * 100 > limit_up_pch:
            observer_sold_info.first_limit_time = trade_time
            logging.info(f"首次涨停 {stock_code} {self.trade_date} {first_limit_time}")
            if observer_sold_info.first_limit_time > high_limit_stock_first_time:
                sell_reason = f'基于高位板{high_limit_stock_first_time}后涨停'
                logging.info(f"卖出 {stock_code} {close} {trade_time} {sell_reason}")
                self.sell_stock(stock_code, close, current_time, observer_sold_info, sell_reason)
                return
        observer_sold_info.pre_avg_price = avg
    ...
    def buy_stock(self, stock_code, stock_name, buy_price, buy_time):
        ...
        if self.observer_map_to_sell:
            for sell_stock_code, observer_sold_info in self.observer_map_to_sell.items():
                if (observer_sold_info.sold_strategy_type & SoldStrategyType.SoldLossStopNeedBuyStock):
                    if not observer_sold_info.is_sell:
                        sell_reason = '基于先卖后买'
                        logging.info(f"卖出 {sell_stock_code} {buy_time} {sell_reason}")
                        self.sell_stock(sell_stock_code, 0, buy_time, observer_sold_info, sell_reason)
    ...
    def sell_stock(self, stock_code: str, sold_price: float, trade_time: datetime.datetime, observer_sold_info: SoldInfoObserver, sell_reason: str):
        if self.run_mode != StrategyTaskMode.MODE_BACKTEST_LOCAL:
    ...
    def export_trade_records_to_excel(self):
        ...
        with Session() as session:
            ...
            for trade_record, observed_record in results:
                ...
                # 获取分钟数据
                next_trade_date = TradeCalendar.get_next_trade_date(buy_date_str)
                minutes_data = get_minute_data(trade_record.stock_code, next_trade_date)
                for minute in minutes_data:
                    if datetime.datetime.strptime(minute.trade_time, "%Y-%m-%d %H:%M:%S").strftime("%H:%M:%S") == "09:30:00":
                        excel_record.extend([minute.close, minute.open, minute.high, minute.low, minute.avg])
                    if datetime.datetime.strptime(minute.trade_time, "%Y-%m-%d %H:%M:%S").strftime("%H:%M:%S") == "09:31:00":
                        excel_record.extend([minute.close, minute.open, minute.high, minute.low, minute.avg])
