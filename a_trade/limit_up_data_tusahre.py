...
import requests
import sys
from settings import _get_tushare
from trade_calendar import TradeCalendar
from stocks_daily_data import get_previous_trade_date, StockDailyData
from sqlalchemy import Column, String, Float, Integer, func
from db_base import Session, get_recent_trade_date_in_table,Base
from trade_utils import code_with_exchange, timestamp_in_millis
import pywencai
import pandas as pd
from trade_utils import time_difference_less_than
from trade_utils import is_10cm_stock
    ...
    def __init__(self, trade_date):
        ...
        with Session() as session:
            ...
            self.limit_failed_map = {
                record.stock_code: record
                for record in result
                if record.limit_status == 'Z'  # 'F' 表示炸板
            }
...
def fetch_reason_types_from_limitpool_for_day(current_date):
    ...
    while True:
        ...
        print(timestamp_in_millis('2024-01-01'))
        params = {
            "page": str(page),
            "limit": "100",
   ...
   # 第一部分：尝试拉取数据
    ...
    try:
        for _, row in res.iterrows():
            ...
            if not stock_code:
                logging.warning(f"数据缺失: 股票代码为空，跳过此条记录")
                continue
...
if __name__ == "__main__":
    ...
    end_date = sys.argv[2]
    # fetch_reason_types_from_wencai_util(start_date)
    ...
    #     if row['name'] == '聚星科技':
    #         print(f"{row['trade_date']} - {row['ts_code']}")

    # with Session() as session:
    # # 分组查询交易日和股票名称，统计股票代码的唯一数量
    #     query = (
    #         session.query(
    #             LimitUpTushare.trade_date,
    #             LimitUpTushare.stock_name,
    #             func.count(func.distinct(LimitUpTushare.stock_code)).label("code_count")
    #         )
    #         .group_by(LimitUpTushare.trade_date, LimitUpTushare.stock_name)
    #         .having(func.count(func.distinct(LimitUpTushare.stock_code)) > 1)
    #     )
    ...
    #     # 打印异常数据
    #     print("以下是同交易日下同名但不同代码的异常数据:")
    #     for anomaly in anomalies:
    #         print(f"交易日: {anomaly.trade_date}, 股票名称: {anomaly.stock_name}, 不同股票代码数: {anomaly.code_count}")